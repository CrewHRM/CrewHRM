...
export function Apply({ job={} }) {
    ...
    const submitApplication = () => {
		request( 'apply_to_job', {application: state.values, nonce, nonceAction}, resp=>{
			const {success, data:{notice, message}} = resp;
	...
	let fields_to_render = is_segment ? fields[state.active_tab] : Object.keys(fields).map((key) => fields[key]).flat();
	let is_next_enabled = isNextEnabled( fields_to_render );
	
    return state.submitted ? <Applied error_message={state.error_message}/> : 
			...
			{(is_segment && (
				...
				<div>
					<button
						...
						title={!is_next_enabled ? __('Please fill all the required fields to submit') : null}
