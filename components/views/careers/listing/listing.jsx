	...
	return <>
		<div data-crewhrm-selector="careers-header">
			<CoverImage src={hero} width="100%" className={'padding-vertical'}>
				<span className={'d-block font-size-38 font-weight-500 line-height-24 letter-spacing--38 color-white padding-vertical-50 margin-top-25 margin-bottom-25'.classNames()}>
					{__( 'Small teams, global mission' )}
				</span>
			</CoverImage>
		</div>
		<div data-crewhrm-selector="job-listing" className={'listing'.classNames(style)} style={{marginTop: '59px'}}>
				...
				<div className={'margin-right-50'.classNames()}>
					{Object.keys(filterList).map(filter_key=>{
						...
						return <div key={filter_key} className={'margin-bottom-23 overflow-auto'.classNames()}>
							<span className={"d-block font-size-14 font-weight-700 line-height-24 letter-spacing--14 color-text-light margin-bottom-16".classNames()}>
								{section_label}
							</span>
							...
							{selection_type=='list' && options.map(option=>{
								...
								return <span key={id} className={`d-block font-size-14 cursor-pointer margin-bottom-18 ${is_active ? 'font-weight-600 color-primary' : 'font-weight-500 color-text-light'}`.classNames()} onClick={()=>setFilter(filter_key, id)}>
									{label} ({count})
								</span>
							}) || null}
						</div>
			...
			<div data-crewhrm-selector="listing" className={'content-area'.classNames(style)}>
				...
				{jobs.map(job=>{
					...
					return <div className={'d-flex align-items-center padding-15 margin-bottom-20'.classNames()}>
						<div className={'flex-1'.classNames()}>
							...
							<span className={'font-size-13 font-weight-400 line-height-24 letter-spacing--13 color-text-light'.classNames()}>
								{meta.map((m, i)=>{
									return <span key={i}>
										{m} 
				...
				<div data-crewhrm-selector="loading" className={'text-align-center'.classNames()}>
					<LoadingIcon/>
					<div className={'font-size-13 font-weight-400 line-height-21 color-text-light margin-top-8'.classNames()}>
						{__( 'Loading More...' )}
					</div>
				</div>
			</div>
