...
export function OverView() {
	...
	return <div data-crewhrm-selector="overview" className={'overview'.classNames(style)}>
		...
		<div>
			<DangerouslySet className={'font-size-15 font-weight-400 line-height-22 letter-spacing-15 color-primary'.classNames()}>
				...
				{education.map(ed=>{
					...
					return <div data-crewhrm-selector="education" key={education_id} className={'d-flex margin-bottom-10'.classNames()}>
						<div style={{width: '150px'}}>
							<span className={'font-size-15 font-weight-400 line-height-24 letter-spacing--15 color-text-light'.classNames()}>
								{new Date(date_from).getFullYear()} - {new Date(date_to).getFullYear()}
							</span>
						</div>
						<div className={'flex-1'.classNames()}>
							<span className={'font-size-15 font-weight-400 line-height-24 letter-spacing--15 color-primary'.classNames()}>
								{degree} <span className={'color-text-light'.classNames()}>at</span> {institute}
							</span>
						</div>
					</div>
				})}
		...
		{qna.length && <>
			...
			{qna.map((q, i)=>{
				...
				return <div key={qna_id}>
					<span className={'d-block font-size-17 font-weight-500 line-height-24 letter-spacing--17 color-primary margin-bottom-1'.classNames()}>
						{question}
					</span>
		...
		{social_links.length && <>
			...
			{[...new Set( social_links )].map(link=>{
				return <a key={link} href={link} rel="noopener noreferrer nofollow" target="_blank" className={'d-inline-block margin-right-20'.classNames()}>
					<i className={getSocialIcon(link) + 'font-size-20 color-primary'.classNames()}></i>
				</a>
			})}
		</> || null}
	</div>
