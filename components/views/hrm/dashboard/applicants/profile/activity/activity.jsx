	...
	return <>
		...
		{attachments.length && <div className={"comment-attachments".classNames(style)}>
			{attachments.map(attachment=>{
				return <CoverImage src={attachment.url} height={50} className={'border-radius-5'.classNames()}/>
			})}
		</div> || null}
	</>
}
	...
	return <div data-crewhrm-selector="activity" className={'activities'.classNames(style)}>
		{acitivities.map((activity, i)=>{
			...
			return [
				...
				<Line key={i+'_2'} show={i<acitivities.length-1} className={'margin-top-20 margin-bottom-20'.classNames()}/>
			]
		})}
	</div>
}